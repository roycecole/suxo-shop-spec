...
## 5. 待決議事項
- [ ] **需要業主決策（非技術判斷）**：若未來出現實際的濫用/超用糾紛，要靠什麼機制舉證（合約條款？人工稽核頻率？）——這是拾夜科技與客戶之間的合約/法務安排，不是本規格庫能單方面定案的技術決策，待業主提供方向後才能具體化（如：合約要不要載明稽核權利、稽核頻率多久一次、由誰執行）
- [ ] **需要業主決策（非技術判斷）**：GMV 計費若要恢復可行性，需要客戶方同意的資料申報機制設計（不透過 ShyeCMS 直接連線取得）——這牽涉具體的抽成比例/計費模式（見 [02-data-model.md](02-data-model.md) §6 GMV 超額抽成計算依據，兩項待決議互相依賴，建議合併決策），屬於商業模式本身尚未定案，不是規格文件能單方面回答的問題
