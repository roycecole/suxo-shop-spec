...
| v0.16 | 2026-09-10 | ordinarycas | 使用者要求「繼續補完 9 unresolved」：重新檢視原本判定「無法解決」的 9 項，發現多數其實可以再往前推進——(1) Order 訂單編號**完全解決**：重新評估後定案維持亂數編號為永久設計（序號式編號代價不划算），並在 `ecommerce-services` 追加防碰撞重試，移出本表；(2) 服務資源消耗**部分解決**：實際跑了本機 `docker stats` 量測（16 容器閒置約 480MiB）與 `ab` 併發測試，取代原本純推測的「需要實測」標記；(3) ShyeCMS 4 項業主決策題各補上具體建議草案或查證過的業界參考數據（非規格代為拍板，草案仍待業主/法務核可）；(4) LINE/Google OAuth 補上排序建議；(5) Payment/Shipping 兩項外部資源阻擋項補上取得憑證/資格後的執行清單，縮短未來實際申請到後的等待時間。統計訂正為 8 項未解決 + 71 項已解決 |
...
| v0.21 | 2026-09-12 | ordinarycas | 訂正「使用說明」段落遺留的落差：v0.20 已把統計數字訂正為「9 項未解決 + 72 項已解決」，但同一段落開頭的統計句沒有跟著一起改，仍停留在 v0.19 當時的「10 項未解決 + 71 項已解決」，兩處數字互相矛盾。本次修正該句與異動紀錄一致；本表 §1–§6 本身無其他異動 |
| v0.22 | 2026-09-12 | ordinarycas | 同步 [17-service-order.md](17-service-order.md) v0.20：§4 新增一列「Order / Shipping」——結帳 Saga 步驟 1.55 運費核對（本輪新增的資安修正）不驗證查得的物流方式是否歸屬本次結帳品項所屬的賣家，來源文件已補上對應 `- [ ]`；本輪同步的其餘 6 項文件落後缺口（07 §3.1/§3.2、08 §3.1、11 §5.1、12/13 WMS 庫存檢查、17/21 運費核對機制本身、17 索引記錄）皆為補寫既有現況或效能修正，未新增/解決任何待決議事項，不影響本表其餘列；統計訂正為 10 項未解決 + 72 項已解決 |
...
統計：**10 項未解決** + 72 項已解決（未解決數逐行核對 §2–§5 表格列數所得，非估算；已解決數為累計手動追蹤，逐項列名核對而非心算，已解決項目不列入本表）。v0.22 新增 1 項（見該版異動紀錄：Order 結帳 Saga 步驟 1.55 運費核對不驗證物流方式所屬賣家，本輪資安修正時的已知相鄰缺口）。新增 2 項（見 v0.18：Order 沒有任何機制會把 SubOrder 轉為 Completed，修正 Analytics 批次拉取缺口時意外發現，非規格代為決定觸發方式即可解決，需要產品層級的設計討論；見 v0.19：結帳冪等性修正的刻意簡化取捨——購物車標記已結帳後永久不可重試，是否可接受待確認）。**v0.16 當輪的 8 項中有 7 項補上了具體的建議草案/執行清單/研究數據，不再是空白的待答問題**（v0.18/v0.19 新增的 2 項為之後獨立發現，尚未經過同一輪處理）：§2 ShyeCMS 4 項業主決策題，各自補上具體機制草案（稽核權利條款、客戶自行申報機制、90 天資料保留政策）或查證過的業界參考區間（GMV 抽成 1–3%）；§3 LINE/Google OAuth 補上排序建議（非阻擋性缺口）、服務資源消耗補上本機實測數字（16 容器閒置僅約 480MiB，遠低於原估算）；§4 Payment/Shipping 兩項外部資源阻擋項各補上取得憑證/合作資格後的具體執行清單。§4 原本第三項「Order 訂單編號」本輪**完全解決**並移出本表——重新評估後認為序號式編號的代價（洩漏營業量、需要額外集中計數基礎設施）不划算，定案維持現有的亂數式編號為永久設計，並在 `ecommerce-services` 追加防碰撞重試機制（詳見 [17-service-order.md](17-service-order.md) §2.1）。近期解決（前一輪 17 項）：Catalog/vendor-admin 稅務欄位、WMS 3 項、Vendor 多賣家管理員角色延後、Order 逾時付款、Payment 退款/對帳設計、Media 2 項、CMS Page Builder、Shipping 溫控物流、Analytics 2 項、Notification 3 項。
...
| 項目 | 來源 | 備註 |
|---|---|---|
| 濫用/超用糾紛的舉證機制（合約條款？人工稽核頻率？） | [01-architecture.md](01-architecture.md) §5 | **需要業主決策**，已補上建議草案（稽核權利條款+頻率），待業主/法務核可 |
| GMV 計費若要恢復可行性，需要客戶方同意的資料申報機制設計 | [01-architecture.md](01-architecture.md) §5 | **需要業主決策**，已補上建議草案（客戶自行申報機制），待核可 |
| GMV 超額抽成的計算依據完全空白 | [02-data-model.md](02-data-model.md) §6 | **需要業主決策**，已補上業界參考區間（1–3%），非規格代為挑選比例 |
| 客戶終止合作的資料交還/刪除政策 | [03-client-lifecycle.md](03-client-lifecycle.md) §7 | **需要業主決策**，已補上建議草案（30 天匯出視窗+90 天保留），待業主/法務核可 |

## 3. 電商平台總覽與前後台需求（[06](06-ecommerce-platform-architecture.md)–[09](09-api-specification.md)）

| 項目 | 來源 |
|---|---|
| 服務數量在單一 VPS 部署下的資源消耗，已補上本機實測數字（16 容器閒置約 480MiB），**正式 VPS 覆核仍待辦** | [06](06-ecommerce-platform-architecture.md) §10 |
| LINE / Google OAuth 實際串接時程，**需要業主決策**（已補上排序建議：非阻擋性缺口，可排在其他議題之後） | [07](07-storefront-requirements.md) §5、[11](11-service-identity.md) §6 |
...
| Order / Shipping | 結帳 Saga 步驟 1.55 運費核對（v0.20 資安修正）不驗證查得的 `ShippingMethodId` 是否歸屬本次結帳品項所屬的賣家——Order 沒有地址/地區欄位可供反推，即使加上檢查也只是半套；**待確認**是否需要至少加上「所屬賣家須為品項賣家之一」的部分檢查，或維持現狀等未來補上地址/地區欄位時一併處理（[17](17-service-order.md) §6、[21](21-service-shipping.md) §4） |
