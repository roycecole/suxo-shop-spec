...
| 7 個服務的列舉欄位序列化格式尚未逐一查證 | 本輪（2026-09-11）統一 15 服務的 Problem Details／列舉 JSON 序列化慣例（[09-api-specification.md](09-api-specification.md) §2.1/§2.2）時，盤點確認 Catalog/Gateway/Payment/Order/CMS/WMS/Vendor 7 個服務的既有列舉欄位格式與前台（`ecommerce-admin`）依賴情形，並逐一套用 canonical camelCase 慣例或記錄為技術債保留原格式。但 Identity/Media/Notification/Shipping/Analytics/Reviews/Promotions 這 7 個服務目前**也有**列舉欄位是底層數字輸出（無任何轉換器，與 WMS/Vendor 修正前是同一類問題，如 Media 的 `MediaAssetDto.StorageProvider`、Notification 的 `SendEmailResult.Status`、Shipping 的 `ShippingMethodDto` 相關欄位、Promotions 的 `CouponListItemDto.DiscountType`/`ScopeType` 等），本輪任務範圍明確只查證了前 7 個服務，未逐一 grep `ecommerce-admin`/`ecommerce-storefront` 是否依賴這些欄位現況的數字格式——不代表已確認安全，是刻意記錄下來、待下一輪查證的殘留缺口，見 [09-api-specification.md](09-api-specification.md) §2.2 表格下方的說明 |
