...
| v0.9 | 2026-09-11 | ordinarycas | §5 `POST /internal/v1/catalog/products/batch` 回應擴充新增 VendorId（資安修正的延續：v0.8 修正 Order Service 結帳 Saga 信任買家 Price 的缺口時，VendorId 同樣直接信任請求本文、供拆 SubOrder／算佣金的相鄰風險未一併處理，稽核後本輪收斂）——商品/變體回應各自新增 VendorId 欄位（變體無獨立 VendorId，取自所屬 Product，比照既有 CategoryIds 規則），供 Order Service 在同一次批次查詢核對買家結帳請求裡的 VendorId 是否遭竄改，不需第二次往返，詳見 [17-service-order.md](17-service-order.md) v0.15 §4 步驟 1.5 擴充說明 |
| v0.10 | 2026-09-12 | ordinarycas | §5 `DELETE /api/v1/vendor/products/{id}` 補上刪除前新增的 WMS 庫存檢查（正確性修正：硬刪除商品前原本完全不查 WMS，商品刪除後 `Inventory`/`StockBatch`/`StockLedger` 會留下已不存在 ProductId 的孤兒資料，賣家後台也再無入口管理/釋放）——新增呼叫 WMS 新端點 `GET /internal/v1/wms/inventory/products/{productId}/has-stock`（[13-service-wms.md](13-service-wms.md) v0.8），WMS 若判定仍有庫存則拒絕刪除並回 409，WMS 不可用時 fail closed 回 503 |
...
| `DELETE /api/v1/vendor/products/{id}` | 賣家刪除/封存商品（有訂單記錄者拒絕刪除，改上下架；WMS 仍有庫存者亦拒絕刪除，v0.10 新增，見下方說明） | 賣家 |
...
| `POST /internal/v1/catalog/products/batch` | 結帳 Saga 內部呼叫（**v0.8 資安修正新增，v0.9 擴充 VendorId**）：批次查詢多個商品/變體**目前的真實售價**（已套用生效中的特價，無設定時回原價）、**所屬分類 ID**與**所屬賣家 VendorId**，取代 Order Service 原本直接信任買家結帳請求 Price／VendorId 計算金額/決定賣家歸屬的缺口，並修正 Promotions 分類限定優惠券因 categoryIds 恆為空而必然判定不符範圍的相關缺口，見下方說明 | 內部（僅限 order-service） |

**`POST /internal/v1/catalog/products/batch` 的定價規則**：有效售價＝`SalePrice`（若有設定）否則 `RegularPrice`——與 `GET /api/v1/products`／`GET /api/v1/products/{slug}` 既有回應欄位的語意一致，沿用既有規則、未另外發明一套判斷。資料模型目前沒有特價生效期間欄位（`SalePriceStartAt`/`EndAt`，見 [08-vendor-admin-requirements.md](08-vendor-admin-requirements.md) §5.2 WooCommerce 匯出已記載的同一項缺口），故「生效中」單純代表「有設定值」，沒有時間窗判斷。變體（`ProductVariation`）沒有獨立的特價概念，直接回傳其 `Price` 欄位；`CategoryIds` 掛在商品本身，變體的 `CategoryIds` 取自所屬 `Product`。**`VendorId`（v0.9 擴充）** 直接回傳商品的 `VendorId` 欄位（Product 本身即為賣家自有實體）；變體同樣沒有獨立的 `VendorId`，取自所屬 `Product`，規則與 `CategoryIds` 一致。查無資料的 ID 不會出現在回應對應的清單中，語意比照 [13-service-wms.md](13-service-wms.md) §5 `POST /internal/v1/wms/inventory/batch` 既有的批次查詢慣例（含相同的單次請求筆數上限設計，見該端點文件）。

**`DELETE /api/v1/vendor/products/{id}` 的刪除前 WMS 庫存檢查（v0.10 新增）**：硬刪除商品前，Handler 在既有的「有訂單記錄者拒絕刪除」檢查之後、真正刪除之前，新增第二道下游檢查——呼叫 WMS Service 新增的 `GET /internal/v1/wms/inventory/products/{productId}/has-stock`（[13-service-wms.md](13-service-wms.md) §5），若判定商品本身或其任一變體仍有庫存（`StockQuantity > 0`），拒絕刪除並回 409；呼叫失敗（WMS 不可用）一律 **fail closed**，回 503 拒絕刪除，不會因為查詢失敗就放行刪除。設計理由：(1) 與既有「有訂單記錄者拒絕刪除」（`IOrderHistoryClient.HasOrdersAsync`）是同一種「查下游、有資料就拒絕」的 fail-closed 慣例，這裡只是再加一種下游檢查；(2) 本平台鎖定「庫存歸屬在 WMS，不在 Catalog」（見 [13-service-wms.md](13-service-wms.md) §1），由 WMS 自己判斷「目前是否還有庫存」並主動拒絕，比 Catalog 單方面下令 WMS 清空資料更符合這個決策的精神——Catalog 不該是決定何時抹除 WMS 資料的一方，所以修法選「WMS 若有庫存則擋下刪除」而非「Catalog 刪除時呼叫 WMS 清除庫存」。查詢範疇涵蓋該商品本身與其全部變體（各自一筆 `Inventory`，共用父商品 `ProductId`）。
