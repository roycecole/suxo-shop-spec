# 03 - 客戶生命週期與維運流程 (Client Lifecycle)

## 異動紀錄
| 版本 | 日期 | 作者 | 說明 |
|---|---|---|---|
| v0.1 | 2026-09-08 | ordinarycas | 初版建立 |
| v0.2 | 2026-09-08 | ordinarycas | 因應決策 C（ShyeCMS 不連接客戶平台）移除 §3 建立內部服務憑證/Entitlement 供 Gateway 查詢的步驟，改為純人工設定說明 |
| v0.3 | 2026-09-08 | ordinarycas | §1 生命週期總覽改用 Mermaid 狀態圖繪製 |
| v0.4 | 2026-09-08 | ordinarycas | §5（暫停）補上與 §3/§4 一致的人工同步提醒——先前漏寫，容易誤讀成切 `ClientFeatureEntitlement.Enabled` 就等於客戶站台自動關站（見 [10-gap-analysis.md](10-gap-analysis.md) §12） |
| v0.5 | 2026-09-10 | ordinarycas | §7 解決 2 項待決議（開通部署現階段維持人工、Suspended 狀態訊息文案定案），第 3 項（終止合作資料政策）標記需要業主決策並補上具體待答子問題，回應「將待決議事項列出來實作」需求 |
| v0.6 | 2026-09-10 | ordinarycas | §7 客戶終止合作資料政策補上具體建議草案（30 天匯出視窗、90 天備份保留、期滿銷毀而非匿名化保留），仍標記需要業主/法務最終核可，非規格代為定案，回應「繼續補完 9 項未解決」需求 |

> 本文件描述拾夜科技員工在 ShyeCMS 上，從一個客戶（如「爸芭樂」）洽談到終止合作的完整操作流程。全程由拾夜科技員工在 ShyeCMS 執行——依決策 B，客戶本身不操作此系統。

## 1. 生命週期總覽

```mermaid
stateDiagram-v2
    [*] --> Prospect: 業務建檔（見 §2）
    Prospect --> Active: 環境開通完成（見 §3）
    Active --> Suspended: 欠費/違規（見 §5）
    Suspended --> Active: 問題解決，人工還原
    Active --> Terminated: 合約到期/終止（見 §6）
    Suspended --> Terminated: 合約到期/終止
    Terminated --> [*]
```

對應 [02-data-model.md](02-data-model.md) 的 `Client.Status`。

## 2. 階段一：建檔（Prospect）

1. 業務（SalesOps）與客戶談定合作後，在 ShyeCMS 建立一筆 `Client` 記錄（名稱、聯絡窗口）。
2. 選擇 `SubscriptionPlan`（入門/成長/企業），建立對應的 `ClientSubscription`（`Status=Active`，但此時客戶部署尚未存在）。
3. 此階段**不建立** `ClientDeployment` 記錄——要等實際開通環境後才建立。

## 3. 階段二：開通部署（Provisioning）

依「新客戶上線 SOP」（環境建置 → 資料庫初始化 → 品牌/主題設定 → 金流物流串接測試 → 教育訓練 → 正式上線），**本輪 ShyeCMS 不做自動化開通**（不含 IaC 自動建環境的腳本觸發），而是：

1. 維運人員依既有 SOP 人工/半自動建立客戶的實際部署環境（見 [06-ecommerce-platform-architecture.md](06-ecommerce-platform-architecture.md) 的獨立微服務電商平台）。
2. 依 `ClientSubscription.PlanId` 對應的 `SubscriptionPlan.DefaultFeatureFlagKeys`，在 ShyeCMS 批次產生該客戶的 `ClientFeatureEntitlement` 商業紀錄（`Source=FromPlan`）——這一步**只更新 ShyeCMS 自己的資料庫**，不會、也無法推送到客戶環境。
3. 維運人員依第 2 步的紀錄，**手動**設定客戶環境自己的功能開關設定檔/環境變數（見 [01-architecture.md](01-architecture.md) §3），完成後才代表功能實際生效。
4. 環境就緒後，在 ShyeCMS 建立對應的 `ClientDeployment` 記錄（網域、核心版本號）供內部盤點——純紀錄用途，非連線設定。
5. `Client.Status` 由 `Prospect` 轉為 `Active`。

> 自動化 Provisioning（腳本一鍵建環境）不在本輪範圍，列入 [05-scope-and-open-items.md](05-scope-and-open-items.md)。第 2、3 步之間存在人工落差（ShyeCMS 紀錄與客戶環境實際設定可能不同步），這是決策 C（不連接）的已知取捨，見 [01-architecture.md](01-architecture.md) §4。

## 4. 階段三：營運中（Active）

- **功能調整**：客戶升級/降級方案，或個別功能需求（如臨時關閉某功能進行維護），由 SupportOps 在 ShyeCMS 調整 `ClientFeatureEntitlement`（`Source=Override`）——這只更新 ShyeCMS 的紀錄，**還需要維運人員額外手動到客戶環境調整設定檔才會實際生效**，兩個動作沒有自動連動（見 [01-architecture.md](01-architecture.md) §3）。
- **用量/超額判斷**：因決策 D（不取得客戶資料），ShyeCMS **沒有**客戶用量數據可看；是否超額、是否該建議升級方案，需 SalesOps 另行向客戶詢問或由客戶自行申報，非本輪範圍。
- **續約**：`ClientSubscription.RenewalAt` 到期前提醒；續約失敗（逾期未繳）則 `ClientSubscription.Status = PastDue`，是否連動調整功能紀錄列為待決議（見 [02-data-model.md](02-data-model.md) §6）。
- **SLA 對應**：客戶回報問題的嚴重度分級與回應時間另訂 SLA 表，不在 ShyeCMS 重複定義（ShyeCMS 不是工單系統）。

## 5. 階段四：暫停（Suspended）

- 適用情境：客戶欠費、違反使用條款、或客戶主動要求暫停營運但保留資料。
- 操作：`Client.Status = Suspended`；建議連動將所有 `ClientFeatureEntitlement.Enabled` 批次設為 `false`（等同全站功能關閉，但**部署環境與資料庫不刪除**）。
- **這一步只更新 ShyeCMS 自己的紀錄，不會、也無法自動關閉客戶站台**（決策 C，零連接）——如同 §3/§4 已提醒過的，實際要讓客戶環境反映暫停狀態，需要維運人員額外**手動**登入該客戶環境調整設定檔/環境變數（見 [01-architecture.md](01-architecture.md) §3）。切掉 `ClientFeatureEntitlement.Enabled` 只是 ShyeCMS 內部合約紀錄的更新，不是執行期開關。
- 可恢復：清償/問題解決後，人工將狀態改回 `Active` 並還原功能開關，同樣需要額外手動同步客戶環境。

## 6. 階段五：終止合作（Terminated）

- 操作：`Client.Status = Terminated`。
- **本輪僅記錄狀態變更**，實際的環境下線、備份保留與刪除流程**不在本輪規劃範圍內**，需要另外制定政策（涉及個資法下的資料保留義務與客戶合約條款，終止合作是整個客戶環境層級，規模與複雜度需要獨立規格）。**商品資料的交還已有部分解法**：客戶可在終止前透過賣家後台的 WooCommerce CSV 匯出功能（[08-vendor-admin-requirements.md](08-vendor-admin-requirements.md) §6）自行取得商品資料，會員/訂單資料的交還仍未解決。

## 7. 待決議事項
- [x] ~~開通部署（Provisioning）是否要自動化，或維持人工 SOP + ShyeCMS 僅作記錄~~——**已解決：現階段維持人工 SOP，不自動化**。理由：開通部署（新客戶上線）是低頻事件（不是每天發生的操作），且牽涉實際主機/網域/憑證等維運工作，自動化前得先有穩定可重複的部署腳本（見 [26-project-structure.md](26-project-structure.md) `ecommerce-launch` 現況——連 CI/CD 都還沒有，見 §7），現階段投入自動化的成本高於「維運人員照 SOP 手動跑一次」的成本。ShyeCMS 僅記錄 `ClientDeployment` 盤點資訊（既有設計），不主動觸發任何部署動作。日後客戶數成長到需要頻繁開通時，再重新評估自動化的效益
- [x] ~~`Suspended` 狀態下客戶站台應顯示什麼訊息給該客戶的買家/賣家~~——**已解決**：
  - 買家看到的頁面（前台任何頁面）：「本商店暫停服務中，造成不便敬請見諒，如有未完成訂單請洽官方 LINE 或客服信箱查詢」——不透露暫停原因（可能是欠費等敏感商業資訊），僅告知現況與既有訂單的查詢管道。
  - 賣家後台登入後的橫幅提示：「您的商店目前處於暫停狀態，請聯繫拾夜科技客服了解詳情」——不在前台頁面向買家揭露這類商業細節，但賣家本人需要知道要主動聯繫誰處理。
  - 技術實作：這是前台/後台各自的靜態文案（比照 [07-storefront-requirements.md](07-storefront-requirements.md)/[08-vendor-admin-requirements.md](08-vendor-admin-requirements.md) 既有的 i18n 字典模式），檢查時機是 middleware/proxy 層讀取 `ecommerce-launch` 部署設定裡的暫停旗標（實際串接機制不在本文件範圍，屬於 `ecommerce-launch` 的部署層設計）。
- [ ] **需要業主決策（非技術判斷，已補上建議草案供快速核可/調整）**：客戶終止合作的資料交還/刪除政策（見第 6 節）——這牽涉個資法下的資料保留義務與實際客戶合約條款，最終仍需業主（建議搭配法務顧問核對台灣個資法的最小必要保留原則）拍板，但為了不讓決策憑空開始，這裡給出一版具體草案：

  1. **會員/訂單資料匯出**：提供。終止合作生效前，開放客戶自行從賣家後台匯出會員清單與訂單紀錄（CSV，比照既有的 WooCommerce 商品匯出模式，見第 6 節），匯出視窗為終止生效日**前 30 天**起算，讓客戶有充足時間下載，逾期視為放棄。
  2. **匯出格式與交付方式**：CSV 檔案，透過賣家後台既有的匯出/下載機制（比照 [08-vendor-admin-requirements.md](08-vendor-admin-requirements.md) 已有的匯出連結簽章網址模式），不透過 Email 附加檔案（避免大檔案寄送失敗或外洩風險）。
  3. **保留期限**：客戶環境下線後，資料庫備份**保留 90 天**（採業界常見緩衝期的上限，給予客戶或拾夜科技自己事後發現問題時還能還原查證的餘裕），90 天後執行清除。
  4. **清除方式：比照本平台既有的「匿名化保留」模式，非物理刪除**——90 天緩衝期滿後，不做逐筆刪除，而是**整份資料庫備份加密封存或直接銷毀**（因為是整個客戶環境層級的下線，不是單一會員帳號的部分資料，[11-service-identity.md](11-service-identity.md) §5.1 那套「保留 `User.Id`、清空個資欄位」的匿名化模式是為了維持其他資料表的外鍵完整性，但客戶環境下線後**整個資料庫都不會再被查詢**，不存在需要維持外鍵完整性的下游資料，用不上那套模式的原始設計動機，這裡直接採更單純的「保留期滿即銷毀備份」）。
  5. **合約條款配套**：上述期限與流程需要明確寫進客戶服務合約（終止條款），不能只是拾夜科技內部的操作 SOP——這部分不是本規格庫的範圍，需要業主與法務將以上草案轉成正式合約文字。
  
  以上草案的關鍵數字（30 天匯出視窗、90 天保留期）是**技術/流程角度給出的合理預設值，不是法律意見**，正式採用前應由業主確認是否符合台灣個資法的最小必要保留原則與拾夜科技既有的其他合約慣例
