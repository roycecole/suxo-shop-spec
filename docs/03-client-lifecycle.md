...
- [ ] **需要業主決策（非技術判斷）**：客戶終止合作的資料交還/刪除政策（見第 6 節）——這牽涉個資法下的資料保留義務與實際客戶合約條款，不是規格文件能單方面決定的事。為了讓決策更具體，這裡列出需要業主回答的子問題（而非直接給答案）：(1) 會員/訂單資料要不要提供匯出（商品資料已有 WooCommerce CSV 匯出可用，見第 6 節）；(2) 若提供，匯出格式與交付方式為何；(3) 客戶環境下線後，資料庫備份要保留多久才真正刪除（業界常見抓 30–90 天緩衝期，但實際天數需搭配合約條款與台灣個資法的最小必要保留原則）；(4) 是刪除還是比照本平台既有的「匿名化保留」模式（[11-service-identity.md](11-service-identity.md) §5.1 會員刪除帳號已用過這個模式，可參考但終止合作是整個客戶環境層級，規模不同，不能直接套用）
