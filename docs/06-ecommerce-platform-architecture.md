...
|---|---|
| vCPU | 4 core |
| 記憶體 | 8GB（每個輕量 ASP.NET Core 服務約 100-200MB 常駐，15 個服務+ Gateway + Node 前台 + Postgres 需要餘裕） |
| 硬碟 | 依商品圖片/影片用量另計，起始建議 40GB SSD |

> 此規格為粗估，正式報價前應以實際 Docker 容器跑起來後的 `docker stats` 量測校正，不可直接採用此表作為銷售承諾。
...
## 10. 待決議事項
- [ ] **需要實測（非規格能解決）**：服務數量（15 個）在單一 VPS 部署下的資源消耗——§6.3 已給出粗估規格，但**未經實測校正**，是否需要先合併部分低流量服務（如 CMS/Reviews）再視成長拆分——這需要在真實客戶規格的 VPS 上實際跑滿 15 服務 + Gateway 觀察 CPU/記憶體用量，規格文件只能標示風險，實際數字要等真的部署才量得出來，維持開放
